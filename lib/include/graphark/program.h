...
load_file(const std::string& filename) -> tl::expected<std::string, Error> {
  const std::ifstream input_file(filename);
...
static auto compile_and_get_shader(const GLenum shader_type,
...
public:
  ...
  SetUniformMatrix(const std::string& uniform_name,
    ...
    if (uniform_location < 0) {
      return tl::unexpected(
          Error{
              .message = "Could not get uniform location of \"" +
                         uniform_name + "\""
          });
    }
  ...
  [[nodiscard]] auto ProgramID() const -> unsigned int { return m_program; }
